...
queryDef.modifyUserById = {'qstr': 'UPDATE users SET username = $2, fullname = $3, email = $4, role = $5, api_key = $6 WHERE user_id = $1', 'readTables': [], 'writeTables': ['users'], 'cached': false};
...
queryDef.getSharedDevicesByUserId = {'qstr': 'SELECT devices.device_id, devices.alias, users.username AS owner FROM devices INNER JOIN shared_devices ON devices.device_id = shared_devices.device_id INNER JOIN users ON devices.api_key = users.api_key WHERE shared_devices.user_id = $1', 'readTables': ['devices', 'shared_devices', 'users'], 'writeTables': [], 'cached': true};
queryDef.addDeviceByUserId = {'qstr': 'INSERT INTO devices(api_key, identifier, alias, fixed_loc_lat, fixed_loc_lon) SELECT api_key, $2, $3, $4, $5 FROM users WHERE user_id = $1 RETURNING *', 'readTables': ['users'], 'writeTables': ['devices'], 'cached': false};
...
queryDef.modifyDeviceById = {'qstr': 'UPDATE devices SET alias = $2, fixed_loc_lat = $3, fixed_loc_lon = $4 WHERE device_id = $1', 'readTables': [], 'writeTables': ['devices'], 'cached': false};
queryDef.modifyDeviceByUserId = {'qstr': 'UPDATE devices d SET alias = $3, fixed_loc_lat = $4, fixed_loc_lon = $5 FROM users u WHERE d.api_key = u.api_key  AND d.device_id = $2 AND u.user_id = $1', 'readTables': ['users'], 'writeTables': ['devices'], 'cached': false};
queryDef.deleteDevices = {'qstr': 'DELETE FROM devices WHERE device_id = ANY($1::int[])', 'readTables': [], 'writeTables': ['devices'], 'cached': false};
queryDef.deleteDevicesByUserId = {'qstr': 'DELETE FROM devices WHERE device_id = ANY($2::int[]) AND api_key IN (SELECT api_key FROM users WHERE user_id = $1)', 'readTables': ['users'], 'writeTables': ['devices'], 'cached': false};
...
queryDef.addSharedUserByUserId = {'qstr': 'INSERT INTO shared_devices (device_id, user_id) SELECT d.device_id, u.user_id FROM devices d, users u WHERE d.api_key IN (SELECT api_key FROM users WHERE user_id = $1) AND u.user_id != $1 AND u.username = $2 AND d.device_id = ANY($3::int[]) AND NOT EXISTS (SELECT * FROM shared_devices WHERE device_id = d.device_id AND user_id = u.user_id)', 'readTables': ['devices', 'users'], 'writeTables': ['shared_devices'], 'cached': false};
...
queryDef.deleteSharedUserByUserId = {'qstr': 'DELETE FROM shared_devices WHERE user_id = (SELECT user_id FROM users WHERE username = $2) AND device_id = ANY($3::int[]) AND device_id IN (SELECT d.device_id FROM devices d INNER JOIN users  u ON d.api_key = u.api_key WHERE u.user_id = $1)', 'readTables': ['devices', 'users'], 'writeTables': ['shared_devices'], 'cached': false};
