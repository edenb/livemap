...
queryDef.findDevicesByUser = {'qstr': 'SELECT devices.*, array_agg(shared_devices.user_id) AS shared FROM devices JOIN users ON devices.api_key = users.api_key LEFT JOIN shared_devices ON shared_devices.device_id = devices.device_id WHERE users.user_id = $1 GROUP BY devices.device_id ORDER BY devices.device_id', 'readTables': ['devices', 'shared_devices', 'users'], 'writeTables': [], 'cached': true};
